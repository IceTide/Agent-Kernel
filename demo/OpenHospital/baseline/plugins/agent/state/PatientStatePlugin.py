"""
Patient State Plugin for Hospital Simulation.
Manages patient runtime state with Redis persistence and checkpoint/resume support.
"""
...
class PatientStatePlugin(StatePlugin):
    """
    State plugin specifically for patient agents.
    Tracks patient workflow state and supports checkpoint/resume.
    """
    ...
    async def load_from_db(self) -> None:
        ...
        redis_key = f"{self.agent_id}:temp_vars:PatientStatePlugin"
        try:
    ...
    async def clear(self) -> None:
        ...
        try:
            await self.redis.delete(f"{self.agent_id}:state")
            await self.redis.delete(f"{self.agent_id}:temp_vars:PatientStatePlugin")
            logger.info(f"[{self.agent_id}] Cleared PatientStatePlugin Redis data")
        except Exception as e:
            logger.warning(f"[{self.agent_id}] Failed to clear PatientStatePlugin Redis data: {e}")
